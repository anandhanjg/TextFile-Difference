...
(async ()=>{
    ...
    try {
      ...
      console.log("OUTPUT :-");
      console.log(`${data.new.length} are files created, ${data.modified.length} files are modified, ${data.deleted.length} files are deleted, ${data.found.length-data.modified.length} files are remain same`)
      console.table(data);
    } catch (error) {
